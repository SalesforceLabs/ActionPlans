...
2.  'Enable Chatter Post' places a post in the related record's Chatter Feed (if Chatter is enabled for that object). If unchecked, Action Plans does not create a Feed Post.;
3.  'Unassigned Task Defaults to Record Owner' ensures that Action Plan Template tasks with a blank 'Assigned To' field go to the related record's owner. If unchecked, tasks with a blank 'Assigned To' field are assigned to the running user. Tasks can be assigned to specific people on Action Plan Templates, which overrides this setting.
4.  'Default Object on Lead Conversion' specifies that if an Action Plan is related to a Lead, whether the Action Plan should be moved to the converted Account, Contact, or Opportunity when the Lead is converted. The default is Contact.
5.  'Action Plans Business Hours ID' and 'Use Business Hours' are reserved for future functionality.

## Weekends: Custom Metadata

Modify Custom Metadata "Weekend Day" to indicate which days of the week are the weekend and how many days to move a task forward or backwards to move it to the end of the previous week or the start of the next week. This allows you to work with a different weekend, or to avoid assigning any task to be due on any specific day of the week. <br/>All weekdays should have 0 in both numeric fields.
...
If an Action Plan is related to a Lead, then specify if the Action Plan should be moved to the converted Account, Contact, or Opportunity when the Lead is converted. The default is Contact. Change this setting by editing Action Plan Settings at either the organization level, a profile level, or a user level.

## Task Record Types

If your org uses Task Record Types, you must specify which Record Type to use for Tasks. This will affect the Task Status values available for each Action Plan. This cannot be changed once an Action Plan or Template is created.
...
5.  Add the following code to the object trigger in **before delete** and **after undelete** contexts:
...
![](/resource/1631298921000/ActionPlan_Resources/about_images/ActionPlanFieldSet.png)</fieldset>
...
![](/resource/1631298921000/ActionPlan_Resources/about_images/Template-Creation.png)
...
1. Navigate to an Accounts list view. Select the Accounts you want to create Action Plans for. Click on the "Create Account Action Plans" button.

![](/resource/1631298921000/ActionPlan_Resources/about_images/Select-Accounts.png)

2. Select the template you want to use by typing the name on the lookup and clicking on it.

![](/resource/1631298921000/ActionPlan_Resources/about_images/Choose-template-step1.png)

3. Enter the information specific to this plan.
    - Note that the Accounts you selected earlier are visible and that the "Assign To" has been completed using the current user.

![](/resource/1631298921000/ActionPlan_Resources/about_images/Choose-template-step2.png)

4. Save the Action Plan.
...
![](/resource/1631298921000/ActionPlan_Resources/about_images/ActionPlan-Detail.png)

5. Verify the Action Plan and the tasks.

6. Complete the first task on the Action Plan.
    - Note that the Action Plan is now in the correct related list and that this Account has one open activity and one closed activity. The third activity in the template is dependent and will be created only when its predecessor is complete.

![](/resource/1631298921000/ActionPlan_Resources/about_images/AccountActionPlan-Detail.png)</fieldset>
...
![](/resource/1631298921000/ActionPlan_Resources/about_images/ProcessBuilder.png)

## Apex

To call the invocable Apex from a Trigger or Apex class, use the following sample code:
...
![](/resource/1631298921000/ActionPlan_Resources/about_images/ActionPlanTemplate-import-1.png)
...
Import is also simple. Navigate to the "Import Template" tab. Select the file you want to import using "Browse" and once you’ve found it click on "Import Template."![](/resource/1631298921000/ActionPlan_Resources/about_images/ActionPlanTemplate-import-2.png)
